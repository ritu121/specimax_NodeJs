...
const controller = require('../../controllers/support.status.controller');
...
const {
  ...
  replaceStatus,
  updateStatus,
} = require('../../validations/support.status.validation');
...
router
   ...
   */
  .get(authorize(ADMIN_USER), validate(listStatuses), controller.list)
  /**
   ...
   */
  .post(authorize(ADMIN_USER), validate(createStatus), controller.create);
...
router
   ...
   * @apiError (Not Found 404)    NotFound Tracker does not exist
   */
  .put(authorize(ADMIN_USER), validate(replaceStatus), controller.replace)
  /**
   ...
   * @apiError (Not Found 404)    NotFound Tracker does not exist
   */
  .patch(authorize(ADMIN_USER), validate(updateStatus), controller.update)
  /**

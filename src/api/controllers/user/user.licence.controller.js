...
const pdfGen = require('./invoicePdf.controller');
...
exports.get = (req, res) => {
  ...
  .then((licence)=>{
     ...
     if(licence){
      return res.send(200, { code: 200, message: 'License  retrieved successfully.', data: licence });
     }
...
exports.download = async(req, res) => {
  // const { license } = req.locals;
  const license = await License.findOne({_id : req.params.licenseId});
  // const baseUrl = `http://50.17.107.208:3004`;
  const  baseUrl = req.protocol+"://"+req.headers.host;
  await pdfGen.generateLicense(license.licenseName?.name,license.issuingState,license.licenseNumber,license.country.name,license.issuingAuthority,license._id, license.picture)
  const path= `${baseUrl}/uploads/${license._id}.pdf`;
  
  // let genratedUrl = path.join(__dirname, `../../../public/uploads/${license._id}.pdf`);
  // console.log('url',genratedUrl)
...
exports.update = (req, res, next) => {
  ...
  const upload = multer({ storage }).single('picture');
    upload(req, res, (err) => {
      ...
      // });
      const license = Object.assign(req.locals.license, updatedLicense);
      // console.log('aaa',req.file,req.files.picture)
      if (license.picture==""){

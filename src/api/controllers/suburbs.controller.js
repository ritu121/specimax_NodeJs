...
const City = require('../models/suburbs.model');
const bcrypt = require('bcryptjs');
const sub = require('australian-suburb-list')
 ...
 * @public
 */
exports.get = (req, res) => res.json({code: 200, message: 'Suburbs list retrieved successfully.', data: req.locals.city.transform()});
...
exports.create = async (req, res, next) => {
  try {
    let suburds = sub.suburb;
    suburds.forEach(e=>{
      let subur = new City({'name':e,'countryId':'62b574f242db98961fecde9f'})
      subur.save()
    })
    ...
    //   }
    //   if(data){
    //     res.json(400, {code: 400, message: 'City already exists try different one.'});
    //   }
    //   else{
...
exports.cities = async (req, res, next) => {
  try {
    ...
    .then((data) => {
      console.log('dd',data)
      res.json({code : 200, message : 'Suburbs list retrieved successfully.', data: data});
    })
    .catch((error) => {

...
const SupportStatus = require('../models/support.status.model');
...
exports.load = async (req, res, next, id) => {
  try {
    const status = await SupportStatus.get(id);
    req.locals = { status };
    return next();
  } catch (error) {
    return next(error);
 ...
 * @public
 */
exports.get = (req, res) => res.json({code: 200, message: 'Status list retrieved successfully.', data: req.locals.status.transform()});
...
exports.create = async (req, res, next) => {
  try {
    const status = new SupportStatus(req.body);
...
exports.list = async (req, res, next) => {
  try {
    const statuses = await SupportStatus.list(req.query);
...
exports.remove = (req, res, next) => {
  ...
  status.remove()
  .then(() => res.json({code : 200, message : 'Status delete successfully.', data : {}}))
    .catch((e) => next(e));
};
